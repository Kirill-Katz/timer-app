...
<template>
  <section class="relative sm:hidden">
    <section class="flex min-h-[calc(100vh-7rem)] flex-col gap-6 py-4">
      <div v-if="app.runningLog" class="rounded-lg border border-sage/30 bg-sage/10 p-3">
        <div class="flex items-center justify-between gap-3">
          ...
          <button class="btn-danger inline-flex h-11 w-32 items-center justify-center gap-2 rounded-xl px-4 font-black tabular-nums" type="button" @click="app.endTimer">
      ...
      <div class="min-h-0 flex-1 pb-24">
        ...
        <p v-if="app.loadingMoreLogs" class="px-1 py-4 text-center text-sm font-bold text-stone-300">Loading older logs...</p>
        <p v-else-if="!app.hasMoreLogs && app.groupedLogs.length" class="px-1 py-4 text-center text-sm font-bold text-stone-500">Reached the end of loaded history.</p>
          ...
          <div v-if="app.taskSheetProjectId" class="flex min-h-0 flex-1 flex-col gap-3">
            ...
            <div v-if="app.taskCreateOpen" class="grid shrink-0 gap-2">
              ...
              <div class="grid grid-cols-2 gap-2">
                ...
                <button class="btn-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 font-black" type="button" @click="app.addMobileTask">
                  <Plus :size="18" /> Add
                </button>
              </div>
            </div>
            ...
            <VirtualScroller class="max-h-[40vh] pb-8" :items="app.taskSheetTasks" :item-height="64" item-key="id" :overscan="8">
              ...
              <template #empty>
                <p class="py-8 text-center text-sm font-bold text-stone-300">No tasks yet.</p>
              </template>
            </VirtualScroller>
          </div>
          <div v-else-if="app.projectCreateOpen" class="grid gap-3">
            <div class="flex items-center justify-between">
              <MobileBackButton label="Back" :compact="false" @click="app.projectCreateOpen = false" />
            </div>
            ...
            <form class="grid gap-2" @submit.prevent="app.addMobileProject">
              ...
              <div class="grid grid-cols-[3.25rem_1fr] gap-2">
                ...
                <button class="btn-primary inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-4 font-black" type="submit">
    ...
    <MobileOverlay :show="app.settingsOpen" content-class="px-3 py-4">
        <section class="min-h-[calc(100vh-2rem)] rounded-lg border border-line bg-panel p-4 shadow-soft">
          <div class="mb-4 flex items-center justify-between gap-3">
            <MobileBackButton label="Back" :compact="false" @click="app.closeSettings" />
          </div>
          <div class="mb-5">
          ...
          <div class="grid gap-3">
            ...
            <MobileSettingsCard label="Sync">
              ...
              <button class="btn-primary mt-3 inline-flex min-h-11 w-full items-center justify-center rounded-xl px-4 font-black" type="button" :disabled="!app.syncState.online" @click="app.synchronizeFromRemote">
                Synchronize
              </button>
            ...
            <button class="btn-danger mt-2 inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-4 font-black" type="button" @click="app.signOut">
    ...
    <MobileOverlay :show="Boolean(app.editingLogId)" z-class="z-[70]" content-class="flex flex-col px-4 py-5">
        ...
        <form class="flex min-h-0 flex-1 flex-col gap-3 overflow-auto" @submit.prevent="app.saveLog">
          ...
          <div class="mt-auto grid gap-2 rounded-lg border border-line bg-panel/95 p-2 shadow-soft backdrop-blur">
            ...
            <button v-if="app.currentEditingLog" class="btn-danger inline-flex min-h-12 items-center justify-center gap-2 rounded-xl px-4 font-black" type="button" @click="app.deleteLog(app.currentEditingLog)">
