  ...
  <section class="mx-auto hidden max-w-7xl gap-4 sm:grid lg:grid-cols-[19rem_1fr]">
    <aside class="rounded-lg border border-line bg-panel p-4 shadow-soft">
      <div class="mb-3 flex items-center justify-between gap-3">
        <h2 class="text-xl font-black">Projects</h2>
        <label class="inline-flex items-center gap-2 text-sm font-bold text-stone-300">
      ...
      <form class="grid gap-2" @submit.prevent="app.addProject">
        ...
        <div class="grid grid-cols-[3.25rem_1fr] gap-2">
          ...
          <button class="btn-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 font-bold" type="submit">
            <Plus :size="18" /> Project
          </button>
        </div>
      </form>
      <div class="mt-4 grid max-h-64 gap-2 overflow-auto pr-1 lg:max-h-[calc(100vh-18rem)]">
        <button v-for="project in app.projects" :key="project.id" class="flex min-h-12 items-center gap-3 rounded-lg border bg-panel px-3 text-left font-bold text-ink" :class="[project.id === app.selectedProjectId ? 'border-sage shadow-[inset_3px_0_0_#7ef2bc]' : 'border-line', project.archived ? 'opacity-55' : '']" type="button" @click="app.selectedProjectId = project.id; app.refreshLocalData()">
    ...
    <section v-else-if="app.editingLogId" class="grid gap-4">
      <section class="flex h-[calc(100dvh-8rem)] flex-col overflow-hidden rounded-lg border border-line bg-panel p-4 shadow-soft">
        <div class="mb-4 flex items-center justify-between gap-3">
          <button class="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-line bg-transparent px-3 font-bold text-ink" type="button" @click="app.closeLogEditor">
            <ArrowLeft :size="18" /> Back
          </button>
        </div>
        <div class="mb-5">
          <p class="mb-1 text-xs font-black uppercase text-rust">Time log</p>
          <h2 class="text-3xl font-black leading-none">Edit entry</h2>
        </div>
    ...
    <section v-else class="grid gap-4 xl:grid-cols-2">
      <section class="rounded-lg border border-line bg-panel p-4 shadow-soft xl:col-span-2">
        <div class="mb-3 flex items-center justify-between gap-3">
          <h2 class="text-xl font-black">Timer</h2>
        </div>
        <div v-if="app.selectedProject" class="mb-4 grid gap-2 rounded-lg border border-line bg-panel p-3 sm:grid-cols-[1fr_3.25rem_2.75rem]">
          <input v-model="app.selectedProject.name" class="min-h-11 rounded-lg border border-line bg-panel px-3 font-bold text-ink" @blur="app.saveSelectedProject" />
        ...
        <div class="flex min-h-32 items-center justify-center gap-3 rounded-lg border border-sage/30 bg-sage/10 text-ink">
          <Clock :size="24" />
          <strong class="text-[clamp(2.35rem,13vw,5.5rem)] font-black leading-none">{{ app.runningLog ? app.formatDuration(app.runningLog.start_time, null) : '00:00:00' }}</strong>
        </div>
        <div class="sticky bottom-3 z-10 mt-4 grid gap-2 rounded-lg border border-line bg-panel/95 p-2 shadow-soft backdrop-blur sm:static sm:grid-cols-[1fr_auto] sm:border-0 sm:bg-transparent sm:p-0 sm:shadow-none">
          ...
          <button v-if="!app.runningLog" class="glass-start inline-flex min-h-12 items-center justify-center gap-2 rounded-lg px-5 font-black disabled:opacity-50" type="button" :disabled="!app.canStartTimer" @click="app.beginTimer">
            <Play :size="18" /> Start
          </button>
          <button v-else class="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-red-700 px-5 font-black text-white" type="button" @click="app.endTimer">
            <Square :size="18" /> Stop
          </button>
        </div>
      </section>
      <section class="rounded-lg border border-line bg-panel p-4 shadow-soft">
        <div class="mb-3 flex items-center justify-between gap-3">
          <h2 class="text-xl font-black">Tasks</h2>
        </div>
        <form class="grid gap-2 sm:grid-cols-[1fr_auto]" @submit.prevent="app.addTask">
          ...
          <button class="btn-primary inline-flex min-h-11 items-center justify-center gap-2 rounded-lg px-4 font-bold disabled:opacity-50" type="submit" :disabled="!app.selectedProjectId">
            <Plus :size="18" /> Task
          </button>
        </form>
      ...
      <section class="rounded-lg border border-line bg-panel p-4 shadow-soft xl:col-span-2">
        <div class="mb-3 flex flex-wrap items-center justify-between gap-3">
          <h2 class="text-xl font-black">Time logs</h2>
        </div>
          ...
          <article v-for="log in app.visibleLogs" :key="log.id" class="grid cursor-pointer grid-cols-[1fr_auto_auto] items-start gap-3 rounded-lg border border-line bg-panel p-3 transition hover:border-sage" tabindex="0" role="button" @click="app.openLogEditor(log)" @keydown.enter.prevent="app.openLogEditor(log)" @keydown.space.prevent="app.openLogEditor(log)">
            <div class="grid min-w-0 gap-1">
              <div class="flex items-center gap-2">
                ...
                <strong class="truncate">{{ app.projectById(log.project_id)?.name ?? 'Unknown project' }}</strong>
              </div>
