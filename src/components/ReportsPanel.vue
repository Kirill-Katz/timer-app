...
type PresetRange = '7d' | '30d' | '180d' | '365d' | 'lifetime' | 'custom';
...
const timelineStartDate = computed(() => {
  if (!props.app.reportLogs.length) return '';
  return toDateInputValue(new Date(props.app.reportLogs[props.app.reportLogs.length - 1].start_time));
});
...
const rangeBounds = computed(() => {
  ...
  if (rangePreset.value === 'custom') {
    ...
    const firstLog = props.app.reportLogs[props.app.reportLogs.length - 1];
...
const reportSeries = computed(() => {
  ...
  for (const log of props.app.reportLogs) {
    const clipped = clipLogToRange(log, bounds.start.getTime(), bounds.endExclusive.getTime());
    if (!clipped) continue;
  ...
  const projectMeta = new Map<string, { name: string; color: string; index: number }>(
    props.app.projects.map((project, index) => [
      project.id,
      {
        name: project.name,
...
const chartData = computed<ChartData<'bar'>>(() => ({
  ...
  datasets: reportSeries.value.projectOrder.map((projectId, datasetIndex, projectIds) => {
    const project = props.app.projects.find((entry) => entry.id === projectId);
    ...
    tooltip: {
      ...
      callbacks: {
        label(context) {
          ...
          return `${context.dataset.label}: ${formatDurationLabel(value)}`;
        },
        beforeBody(items) {
          const bucket = reportSeries.value.buckets[items[0]?.dataIndex ?? -1];
          return bucket ? [`Total: ${formatDurationLabel(bucket.hours)}`] : [];
        }
      }
    }
  },
  scales: {
    ...
    y: {
      ...
      ticks: {
        color: '#d6d3d1',
        callback(value) {
          return formatDurationAxisLabel(Number(value));
        }
      }
    }
  }
...
const totalLabel = computed(() => props.app.formatDurationMs(reportSeries.value.totalMs));
const hasData = computed(() => reportSeries.value.buckets.length > 0);
const hasLogs = computed(() => props.app.reportLogs.length > 0);
...
function clipLogToRange(log: TimeLog, rangeStart: number, rangeEnd: number) {
  const logStart = new Date(log.start_time).getTime();
  const logEnd = log.end_time ? new Date(log.end_time).getTime() : props.app.ticker;
...
function formatTitleDate(value: Date) {
  return new Intl.DateTimeFormat(undefined, {
    day: 'numeric',
    month: 'long',
    year: 'numeric'
...
function formatDurationLabel(hours: number) {
  return props.app.formatDurationMs(Math.round(hours * 3_600_000));
}

function formatDurationAxisLabel(hours: number) {
  const formatted = props.app.formatDurationMs(Math.round(hours * 3_600_000));
  return formatted.slice(0, 5);
}

function normalizeProjectColor(project: Project | undefined) {
  return project?.color || '#7ef2bc';
}
